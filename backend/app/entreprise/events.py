"""F11 — Pub/sub in-process pour les événements entreprise.updated.

Un canal par account_id (asyncio.Queue par abonné). Pas de broker externe en
MVP (acceptable pour 1 worker FastAPI).
"""
...
async def publish(account_id: UUID | str, payload: dict[str, Any]) -> int:
    ...
    for q in queues:
        try:
            ...
            logger.warning("entreprise events queue full for account=%s", aid)
    return delivered


def publish_sync(account_id: UUID | str, payload: dict[str, Any]) -> None:
    """Variante non-async best-effort. Drop si pas de loop."""
    try:
        ...
        pass
    logger.debug("publish_sync: no running loop, event dropped for account=%s", account_id)
